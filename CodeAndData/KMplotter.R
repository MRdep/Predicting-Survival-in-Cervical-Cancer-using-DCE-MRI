...
# read in tracer kinetic and clinical parameters
  ...
  splitpoints.Fp <- sort(model.params$Median.Fp.2CXM)[19]
  splitpoints.PS <- sort(model.params$Median.PS.2CXM)[24]
  splitpoints.vp <- sort(model.params$Median.vp.2CXM)[22]
  splitpoints.ve <- sort(model.params$Median.ve.2CXM)[18]
  splitpoints.Ktrans <- sort(model.params$Median.Ktrans.2CXM)[26]
  ...
  splitpoints.Age <- sort(clinical.params$Age)[18]
  splitpoints.Vol <- sort(clinical.params$Vol)[26]
...
# 2cxm parameters 
for (i in 1:len){
  if(model.params$Median.Fp.2CXM[i] < splitpoints.Fp){
...
for (i in 1:len){
  if(model.params$Median.PS.2CXM[i] < splitpoints.PS){
  ...
  median.vp.2CXM <- median(model.params$Median.vp.2CXM)
  for (i in 1:len){
    if(model.params$Median.vp.2CXM[i] < splitpoints.vp){
  ...
  for (i in 1:len){
    if(model.params$Median.ve.2CXM[i] < splitpoints.ve){
  ...
  for (i in 1:len){
    if(model.params$Median.Ktrans.2CXM[i] < splitpoints.Ktrans){
...
for (i in 1:len){
  if(clinical.params$Age[i] < splitpoints.Age){
...
for (i in 1:len){
  if(clinical.params$Vol[i] < splitpoints.Vol){
  ...
  
{
  ...
  predictor[ones] = 0
  atrisklabs = c('\u2265 50th percentile', '< 50th percentile')
  pvalheight = -50.
...
if (param =='FE')
{
  ...
  ones <-which(predictor == 2)
  predictor[ones] = 0
  atrisklabs = c('\u2265 64th percentile', '< 64th percentile')
  pvalheight = -50.
  atrisk.line = c(bottomline,topline)
...
if (param =='Ktrans')
{
  ...
  ones <-which(predictor == 2)
  predictor[ones] = 0
  atrisklabs =c('\u2265 69th percentile', '< 69th percentile')
  pvalheight = -50.
  atrisk.line = c(bottomline,topline)
...
if (param =='2CXMvp'){
  ...
  ones <-which(predictor == 2)
  predictor[ones] = 0
  atrisklabs = c('\u2265 58th percentile', '< 58th percentile')
  pvalheight = -50.
  atrisk.line = c(bottomline,topline)
...
if (param =='2CXMve'){
  ...
  ones <-which(predictor == 2)
  predictor[ones] = 0
  atrisklabs = c('\u2265 47th percentile', '< 47th percentile')
  pvalheight = -50.
  atrisk.line = c(bottomline,topline)
...
if (param =='vol')
{
  ...
  titleForPlot = expression('Tumour volume')
  location = 'bottomleft'
  atrisklabs = c('< 69th percentile', '\u2265 69th percentile')
  pvalheight = -50.
  atrisk.line = c(bottomline,topline)
...
if (param =='age')
{
  labelsforLeg1 = '< 47th percentile'
  labelsforLeg2 = '\u2265 47th percentile' 
...
if (param =='stage')
{
  labelsforLeg1 = c('Stage T1/T2a')
  labelsforLeg2 = c('Stage T2b/T4')
  predictor = as.numeric(factor(classStage))
  titleForPlot = expression('T stage')
  location = 'bottomright'
  atrisklabs = c('Stage T1/T2a','Stage T2b/T4')
  pvalheight = -50.
  atrisk.line = c(bottomline,topline)
